...
public final class ClassLoaderMasks {
    ...
    public static Class<?> defineClass0(
        ...
        if (buffer != null) {
            ...
            return ClassLoaderAccessor.defineClass0(
                    loader, lookup, name, instrumented, 0, instrumented.length, domain, initialize, flags, classData);
        }
        return ClassLoaderAccessor.defineClass0(
                loader, lookup, name, b, off, len, domain, initialize, flags, classData);
    }
    ...
    public static Class<?> defineClass1(
        ...
        if (buffer != null) {
            byte[] instrumented = GaletteTransformer.getInstanceAndTransform(buffer, false);
            return ClassLoaderAccessor.defineClass1(loader, name, instrumented, 0, instrumented.length, pd, source);
        }
        return ClassLoaderAccessor.defineClass1(loader, name, b, off, len, pd, source);
    }
    ...
    public static Class<?> defineClass2(
        ...
        if (b != null && off >= 0 && len >= 0 && len + off <= b.limit()) {
            ...
            buffer.rewind();
            return ClassLoaderAccessor.defineClass2(loader, name, buffer, 0, instrumented.length, pd, source);
        }
        return ClassLoaderAccessor.defineClass2(loader, name, b, off, len, pd, source);
    }
}
